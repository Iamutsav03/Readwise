...
export default function FlashReviewTab({ vocabulary, onReview, onFinish }) {
  ...
  if (!currentWord) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", padding: 32 }}>
        <div style={{ width: 64, height: 64, background: "var(--rw-page-accent-muted)", color: "var(--rw-accent)", borderRadius: 32, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24 }}>
          <Check size={32} />
        </div>
        ...
        <button onClick={onFinish} style={{ padding: "12px 24px", background: "var(--rw-page-card-bg)", border: "1px solid var(--rw-page-border)", borderRadius: 8, color: "var(--rw-page-text)", fontWeight: 600, cursor: "pointer" }}>Back to Queue</button>
      </div>
    );
  }
  ...
  const cardStyle = {
    ...
    borderRadius: 16, background: "var(--rw-page-card-bg)", border: "1px solid var(--rw-page-border)",
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", padding: isMobileOrSmaller ? "8px 8px 0" : "12px 0 0", alignItems: "center" }}>
      ...
      >
        ...
        <div style={{ ...cardStyle, transform: flipped ? "rotateY(0deg)" : "rotateY(-180deg)", cursor: "grab" }}>
          ...
          <div style={{ padding: 12, background: "var(--rw-page-hover-bg)", border: "1px solid var(--rw-page-border)", borderRadius: 8, width: "100%", textAlign: "center" }}>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  background: "var(--rw-page-card-bg)", border: `1px solid var(--rw-page-border)`, borderRadius: 12,
                  minHeight: 56, cursor: "pointer", transition: "transform 0.1s"
                }}
                onMouseEnter={e => e.currentTarget.style.border = `1px solid ${btn.color}`}
                onMouseLeave={e => e.currentTarget.style.border = `1px solid var(--rw-page-border)`}
              >
                ...
                {!isMobileOrSmaller && <span style={{ position: "absolute", top: 4, right: 6, fontSize: 10, color: "var(--rw-page-text-mute)", background: "var(--rw-page-hover-bg)", padding: "2px 6px", borderRadius: 4 }}>{btn.num}</span>}
              </button>
            ))}
          </div>
        )}
